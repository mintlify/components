const getParentTreeItem = (element: HTMLElement, container: HTMLElement): HTMLElement | null => {
    const findParent = (parent: HTMLElement | null): HTMLElement | null => {
        ...
        if (parent.getAttribute('role') === 'group') {
            ...
            if (folder instanceof HTMLElement && folder.getAttribute('role') === 'treeitem') {
                return folder;
            }
        }
...
const isInsideCollapsedGroup = (el: HTMLElement, container: HTMLElement): boolean => {
...
const getVisibleTreeItems = (container: HTMLElement): HTMLElement[] =>
    Array.from(container.querySelectorAll<HTMLElement>('[role="treeitem"]')).filter(
        (el) => !isInsideCollapsedGroup(el, container)
    );
...
const getSiblingFolders = (element: HTMLElement, container: HTMLElement): HTMLElement[] => {
    ...
    return Array.from(parentGroup.children).flatMap((child) => {
        const nested = child.querySelector<HTMLElement>(':scope > [role="treeitem"][aria-expanded]');
        const direct =
