...
import React, { ReactNode, CSSProperties, isValidElement } from 'react';

import { Classes } from '@/lib/local/selectors';
import { cn } from '@/utils/cn';

/**
 * Enhance video elements with autoplay-friendly props.
 * When a video has autoPlay, it will also get playsInline, loop, and muted
 * to ensure proper playback behavior across browsers.
 */
...
export interface FrameProps {
  /**
   * Title displayed above the frame content with an icon.
   * Previously named `hint`.
   */
  title?: string;
  /**
   * Description/caption displayed below the frame content.
   * Previously named `caption`.
   */
  description?: string;
  /**
   * CSS styles applied to the frame container.
   */
  style?: CSSProperties;
  /**
  ...
  /**
   * Content to be displayed inside the frame.
   * Video elements with autoPlay will automatically get playsInline, loop, and muted props.
   */
  children?: ReactNode;
}
  ...
  return (
    <div className={containerClassName} data-component-part="frame-wrapper">
      {title && (
        ...
        >
          <svg
            ...
            aria-hidden="true"
            data-component-part="frame-title-icon"
          >
          ...
          <p
            className="text-gray-700 text-sm font-medium dark:text-gray-200"
            data-component-part="frame-title"
          >
            {title}
          </p>
        </div>
      ...
      >
        ...
        />
        <div
          className="relative rounded-xl overflow-hidden flex justify-center"
        ...
        <div
          className="absolute inset-0 pointer-events-none border border-black/5 rounded-2xl dark:border-white/5"
          aria-hidden="true"
