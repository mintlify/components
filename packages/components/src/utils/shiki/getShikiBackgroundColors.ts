...
export const getBackgroundColorsFromHtml = (
    ...
    if (styles.length < 2) return defaultBgColors;
    const backgroundColor = styles[1]?.split('background-color:')[1]?.split(';')[0];
    const shikiDarkBgValue = styles[1]?.split('--shiki-dark-bg:')[1]?.split(';')[0];
    return {
        light: isTransparentOrUndefined(backgroundColor) ? defaultBgColors.light : backgroundColor,
        dark: isTransparentOrUndefined(shikiDarkBgValue) ? defaultBgColors.dark : shikiDarkBgValue,
    };
};
