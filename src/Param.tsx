...
export function ParamField({
  ...
  return (
    <div className={clsx('pb-3 mb-4 border-b border-zinc-100 dark:border-zinc-800')}>
      <div className="flex font-mono text-sm">
        {name && (
          <div className="py-0.5 flex-1 space-x-2 truncate">
            ...
            {required && <span className="text-slate-500 dark:text-slate-300">Required</span>}
            {optional && <span className="text-slate-500 dark:text-slate-300">Optional</span>}
            {defaultValue && (
              <span className="text-slate-500 dark:text-slate-300">Default: {defaultValue}</span>
            )}
          </div>
        )}
        {type && <div className="text-slate-600 dark:text-slate-300">{type}</div>}
      </div>
      <div className="mt-2 prose-sm prose-slate dark:prose-dark">{children}</div>
    </div>
  );
}
